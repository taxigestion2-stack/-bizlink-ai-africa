...
-- Nommée current_user_role() (et non current_role()) car CURRENT_ROLE est un
-- mot réservé du langage SQL standard (comme CURRENT_USER, CURRENT_DATE) :
-- PostgreSQL refuse qu'on l'utilise avec des parenthèses comme un appel de
-- fonction classique, ce qui provoquait l'erreur de syntaxe.
...
create policy "org_update_own_admin" on organizations for update
  using (id = current_organization_id() and current_user_role() = 'admin');
